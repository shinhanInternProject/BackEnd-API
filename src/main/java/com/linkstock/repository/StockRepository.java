...
public interface StockRepository extends JpaRepository<Stock, String> {
    ...
    @Query(value = "select s.stock_code, s.stock_name, s.day_range, s.y_close from stock s where s.stock_code in :stockList order by s.day_range desc limit 20", nativeQuery = true)
    List<String[]> findOrderByEarn(@Param("stockList") List<String> stockList);
}
